...
?>
    ...
    if($cart->total_items() > 0){
        echo "<h4><center>Cart</center></h4><br>
        <div class='container-fluid cartt' style='margin:20px;'>";
        //get cart items from session
        $cartItems = $cart->contents();
        foreach($cartItems as $item){ ?>
            
                <div class="row" style="padding-top:10px; border-bottom:1px solid lightgrey;">
                ...
                <input type="text" class="cuisine" name="rec_cuisine[]" value="<?php echo $item['rec_cuisine']; ?>" readonly style="display:none">
                <input type="text" class="category" value="<?php echo $item['category']; ?>" readonly style="display:none">

                    <div class="col-sm-4 col-xs-4">
                        <p style="font-size:13px;"><?php echo $item['rec_name']; ?><br><span style="font-size:10px; color: grey; font-weight:bold;">(<?php echo $item['rec_serving']; ?> serving)</span></p>
                    ...
                    <div class="col-sm-5 col-xs-5 col-md-5">
                       <div class="row">
                           <div class="col-sm-1"><button type="button" class="btnminus btn text-center" style="" value="<?php echo $item["rowid"]; ?>">-</button></div>
                           <div class="col-sm-2"><input type="text" name="rec_qty[]" class="text-center" id="<?php echo $item['rowid']; ?>" value="<?php echo $item['qty']; ?>" style="background-color:white; border:none; width:30px" readonly></div>
                           <div class="col-sm-1"><button type="button" class="btnplus btn text-center"  style="" value="<?php echo $item["rowid"]; ?>">+</button></div>
                           <div class="col-sm-7">&thinsp;<span style="font-size:12px;">X ₹<?php echo $item["price"]; ?></span></div>
                       </div>
                    </div>
    ...
    <div class="panel panel-default">
      <a data-toggle="collapse" href="#collapse1" style="text-decoration:none; color:black;"><div class="panel-heading">
        <h4 class="panel-title">
          Total Price <span style="font-size:11px; color: grey;">(Inc. all Tax)</span> : ₹<input type="text" id="total_price"  readonly step="0.01" value="<?php echo $total; ?>" size="6" readonly style="border:none;">
		  		<?php 
                if($discount_percentage == 0){ ?>
                    <span class="pre_order_discount_div" style="padding:2px; font-size:13px; color: green; display: none; float:right">
                ...
                <?php
                    ...
                    <span class="pre_order_discount_div" style="padding:2px;font-size:13px; color: green; float:right">
                        Pre-order applied<input readonly style="border: none; display: none;" type="text" size="6" class="pre_order_discount" name="pre_order_discount" value="<?php echo $discount; ?>">
                    </span>
                <?php
                } ?>
...
if($flag == 0 && $cart->total_items() > 0){ ?>
    <center><button type="button" class="btn login_btn" style="border-radius:0; padding:10px 20px; background-color:#D2691E; color:white;">Checkout</button></center>
<?php 
} ?>
  </div>
